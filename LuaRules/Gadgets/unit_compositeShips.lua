...
-- If Spring.CreateUnit is used to generate ships, it may eventually happens
-- that ship is finished before the child unit is even created, so the loading
-- system would fail. Thus, we are storing whether the ship is finished or not
-- in order to disable or not the gun
local boatFinished = {} -- unitID = bool
...
function gadget:UnitLoaded(unitID, unitDefID, unitTeam, transportID, transportTeam)
	if childCache[unitID] then
		if transportID and GG.boatMothers[transportID] then
			...
			if boatFinished[unitID] == true then
				-- The ship was already created and finished, so we can safely wake up the gun
				DisableChild(unitID, false)
			end
		else
